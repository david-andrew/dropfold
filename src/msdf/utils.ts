...
export function computeBox(positions: Float32Array, output: { min: THREE.Vector3, max: THREE.Vector3 }) {
  bounds(positions);
  output.min.set(box.min[0], box.min[1], 0);
  output.max.set(box.max[0], box.max[1], 0);
}

export function computeSphere(positions: Float32Array, output: { center: THREE.Vector3, radius: number }) {
  bounds(positions);
